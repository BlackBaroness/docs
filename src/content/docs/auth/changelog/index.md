...
## 3.39.0 (17.9.2026)
- :::more[Переработана система хранения паролей. Старые пароли совместимы.]
  Старые пароли использовали первые 8 байт как fingerprint для определения формата.
  Новые пароли используют для этой же цели 2 байта, уменьшая пароли с 49 до 43 байт (-12%).

  Внутренняя организация форматов паролей стала более безопасной и эффективной, улучшая производительность
  и снижая риск проблем в будущем (найденных проблем не было, это предосторожность).

  Реализации части алгоритмов пересмотрены и заменены. 
  BaronessAuth использует много разных алгоритмов для обратной совместимости, многие реализации оставались на своём месте чисто по историческим причинам.
  Например, плагин использовал SHA512 от JVM, хотя он не гарантируется Java SE, то есть плагин мог быть несовместим с альтернативными JVM типа OpenJ9.

  Теперь для SHA1 и SHA256 используется JVM, для остального BouncyCastle, а для совсем специфичного уже отдельные решения. 
  К примеру, прежде кое-где использовались Guava Hashing и JackSum.

  Все форматы паролей покрыты unit-тестами, что гарантирует их корректность ещё на этапе сборки, это делает обновления безопаснее.

  Для сравнения массивов байт в паролях теперь используется constant-time алгоритм вместо fail-fast.
  :::
...
## 3.38.0 (23.8.2026)
#### ◇ Общие изменения
- Дефектный пароль в бекапе версии `2.11.0` больше не останавливает всю загрузку. Проблемный профиль импортируется без пароля, в лог попадает предупреждение, а остальные данные продолжают загружаться.
- Плагин аккуратнее переживает ошибки при неудачной перезагрузке или выключении отдельных компонентов. Неудачный `/auth reload` всё ещё не стоит считать нормой для продакшна, но теперь его последствия реже требуют полного рестарта.
- Обновлены внутренние библиотеки.

#### ◇ Встроенные сервера
- Пересмотрены JVM-флаги серверов на Java (`paper` и `nanolimbo`), чтобы экономнее использовать ЦП и ОЗУ и быстрее обслуживать подключения игроков. 
- Обновлена Java в источнике `automatic`.
- Обновлены серверы Paper `1.8.8`, `1.16.5` и `latest`. Версия `1.16.5` переехала с заброшенного Tuinity на [поддерживаемый форк Purpur](https://github.com/OverwriteMC/purpur-1.16.5-continued).
- :::more[Оптимизирован встроенный сервер `paper (latest)`.]
  - Пересмотрены настройки ядра и FAWE, чтобы снизить потребление ЦП и ОЗУ.
  - На поддерживаемых системах используются `FMA` и `io_uring`.
  - Чанки схематики удерживаются в ОЗУ, поэтому постройка больше не должна появляться перед игроком по частям.
  :::
- Новый режим Forwarding `auto` сам выбирает способ связи с прокси, ориентируясь на окружение. Он используется в новых конфигах по умолчанию и не затрагивает уже настроенные серверы.
- Серверы теперь переживают несколько неудачных проверок доступности, а не перезапускаются сразу. Допустимое число ошибок вынесено в `advanced.yml`; по умолчанию перезапуск происходит после второй.
- После `/auth reload` серверы заново регистрируются в BungeeCord. Например, это возвращает их в список после `/nullcordx servers reload`.
- Ненужные папки встроенных серверов теперь автоматически удаляются.

#### ◇ API
- Добавлен метод `sendDiscordMessage`.
- :::more[Добавлен способ аутентификации `bypass`.]
  Аддон может сам решить, нужно ли пропускать игрока. Проверка может завершиться сразу или продолжаться асинхронно, пока игрок ждёт на `auth`. По умолчанию способ выключен.
  [Инструкция и примеры](../guides/api/bypass/).
  :::

## 3.37.0 (29.7.2026)
#### 📦 Обновлено
- Встроенный сервер `picolimbo` (снова совместим со старыми glibc).
- Встроенный сервер `paper (1.8.8)`.
- Встроенный сервер `paper (latest)`.
- ViaVersion, ViaBackwards для встроенных серверов `paper`.
- Поставщик Java `automatic` для встроенных серверов `paper` и `nanolimbo`.
- Внутренние библиотеки.

## 3.36.0 (3.7.2026)
#### ✨ Новое
- В `advanced.yml` добавлена секция `delay-before-sending-first-visuals`, которая позволяет настроить,
  с какой задержкой игрок будет видеть ту или иную визуальную функцию (chat, title и bossbar).
  Это исправляет проблему, когда первое сообщение в чате не было видно, потому что оно отправлялось слишком рано.
  Кроме того, теперь повторные bossbar появляются без задержки. Ранее они появлялись с задержкой 2 секунды, даже если игрок уже давно вошёл. 
#### 📦 Обновлено
- Встроенный сервер `paper (1.8.8)`.
- Встроенный сервер `paper (latest)`.
- Внутренние библиотеки.
...
## 3.35.0 (29.6.2026)
#### ✨ Новое
- В API добавлен метод `createPasswordFromBcryptMcfString`. 
  Он позволяет превратить типичную BCrypt MCF строку в `ProfilePassword`.
  Это в основном нужно для самописных миграций.
#### 📦 Обновлено
...
## 3.34.1 (21.6.2026)
#### 🔧 Исправлено
- Миграции с NLogin, LibreLogin не сохраняли последний логин. Теперь сохраняются и первый, и последний.
#### 📦 Обновлено
- Внутренние библиотеки.
...
- Переработаны некоторые пережитки начала `3.x.x` версии: улучшена система сборки, удалён более неактуальный код. 
  Это едва ли влияет на продакшн, но плагин теперь в целом чуть меньше и чуть чище.
#### 🔧 Исправлено
...
## 3.33.0 (15.6.2026)
#### ✨ Новое
- У команды `/auth find player by ip` теперь есть страницы. 
  Изначально не задумывалось, что у неё будет вывод на тысячи строк,
  что привело к ошибкам при выполнении и большому расходу ОЗУ на крупных серверах.
  Скриншот ниже.
#### 🔧 Исправлено
- В некоторых случаях, скорее как следствие недоработок BungeeCord и других плагинов, BaronessAuth мог бесконечно
  накапливать некоторые данные в ОЗУ (утечка памяти). Теперь этой проблемы нет, вне зависимости от окружения.
#### 📦 Обновлено
- Внутренние библиотеки.
- Встроенный сервер `paper (1.8.8)`.
- Встроенный сервер `paper (latest)`.

![Постраничный вывод команды auth find player by ip](/docs/assets/auth/changelog/3.33.0/find-player-by-ip-pagination.webp)

- ## 3.32.0 (12.6.2026)
#### ✨ Новое
- Добавлена миграция с LibreLogin — вот [инструкция](/docs/auth/guides/migration/librelogin/).
- Включены небольшие peephole оптимизации байткода, чтобы плагин работал чуть-чуть быстрее в некоторых местах.
- Теперь в `advanced.yml` можно включить, чтобы мгновенные методы аутентификации (session, premium) держали игрока в
  `auth` какое-то время, а не сразу пускали в `lobby`. Видео ниже.
...
#### 🔧 Исправлено
- В `general.yml`, в секции `automatic-backups` был параметр для контроля уровня сжатия бекапов.
  Он никогда не использовался, плагин всегда выставлял уровень 9, что требовало слишком много ОЗУ для создания файла.
  Этот параметр перенесён в `advanced.yml` и теперь по умолчанию равняется `6` - бекапы будут создаваться быстрее,
  требовать меньше ОЗУ, а файлы станут немного больше (вы можете отрегулировать это значение, чтобы уменьшить файлы
  вновь).
#### 📦 Обновлено
- Внутренние библиотеки.
- Встроенный сервер `paper (1.8.8)`.
...
<video controls="controls" autoplay loop muted>
    <source src="/docs/assets/auth/changelog/3.33.0/instant-methods-delay.webm" type="video/webm" />
    Your browser does not support this video.
</video>

## 3.31.0 (1.6.2026)
#### ✨ Новое
- Теперь отключение от сервера не приводит к инвалидации кода привязки.
- Теперь `confirmation` в ботах работает, даже если игрок оффлайн (сообщение с кодом отправит при следующем входе).
#### 📦 Обновлено
- Внутренние библиотеки.
- Встроенный сервер `paper (1.8.8)`.
- Встроенный сервер `paper (latest)`.
...
#### 🔧 Исправлено
- Командам `/register` и `/login` больше не назначаются права, когда автоматическое управление правами отключено.
  Так как права назначались всегда, при выключении управления игроки не имели их и не могли пользоваться командами.
  Но большинству серверов управление выключать и не стоит.
#### 📦 Обновлено
- Внутренние библиотеки.
- Встроенный сервер `paper (latest)`.
...
- Доработано автоматическое управление правами игроков:
  - Добавлены настройки в `general.yml`. Теперь можно настроить, какие права будут доступны до логина
  и отняты после него.
  - Теперь автоматически управляет правами на `/register` и `/login`, 
  благодаря чему эти команды не показываются в tab complete после входа.
...
#### 🔧 Исправлено
- Когда были внесены значительные изменения в конфиг бота, его перезагрузка была "полной" и в таком случае
  оставались минорные утечки памяти и дубликаты уведомлений. Рестарт BungeeCord всегда исправлял это, 
  проблема была только в перезагрузке.
...
## 3.25.0 (23.3.2026)
#### ✨ Новое
- Добавлена миграция с JPremium — вот [инструкция](/docs/auth/guides/migration/jpremium/).
...
![Новые раздельные кнопки включения и выключения функций бота](/docs/assets/auth/changelog/3.23.0/bot-buttons.webp)
#### 🔧 Исправлено
...
- Все боты теперь [поддерживают указание прокси](/docs/auth/guides/proxy/), чтобы обходить региональные блокировки их сайтов.
...
## 3.11.0 (17.10.2025)
#### ✨ Новое
- Добавлено новое API — [инструкция](/docs/auth/guides/api/). API было и раньше, но с выходом 3.0.0 было удалено.
#### 🔧 Исправлено
...
- Добавлена миграция с nLogin — вот [инструкция](/docs/auth/guides/migration/nlogin/).
...
<video controls="controls" autoplay loop muted>
    <source src="/docs/assets/auth/changelog/3.2.0/find-login-by-player.webm" type="video/webm" />
    Your browser does not support this video.
</video>
- Пересмотрены и обновлены многие админские сообщения, чтобы привести их в единых стиль и сделать более понятными:
<video controls="controls" autoplay loop muted>
    <source src="/docs/assets/auth/changelog/3.2.0/backup.webm" type="video/webm" />
    Your browser does not support this video.
</video>
...
## 3.0.0 (9.9.2025)
Это обновление огромное: вот [отдельная статья](/docs/auth/changelog/3-0-0/) про него.
...
![Интерфейс BaronessAuth версии 0.2.0-SNAPSHOT-1](/docs/assets/auth/changelog/0.2.0/snapshot-1.webp)

![Интерфейс BaronessAuth версии 0.2.0-SNAPSHOT-2](/docs/assets/auth/changelog/0.2.0/snapshot-2.webp)
...
![Пример автоматической миграции AuthMe в версии v220](/docs/assets/auth/changelog/v220/migration-example.webp)
![Результат автоматической миграции AuthMe в версии v220](/docs/assets/auth/changelog/v220/migration-result.webp)
...
![Сообщение BaronessAuth версии v216](/docs/assets/auth/changelog/v216/message.webp)
...
![Экран ожидания регистрации в BaronessAuth v215](/docs/assets/auth/changelog/v215/registration-screen.webp)

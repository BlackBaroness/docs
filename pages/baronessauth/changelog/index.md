...
- Теперь можно задавать лимит на максимум потоков, которым плагин пользуется для хеширования паролей. Это нужно, чтобы в случае наплыва игроков не лагали остальные программы на машине - хеширование занимает немало процессорного времени.
- Теперь можно задавать лимит на максимум потоков, которым плагин пользуется для общения с базой данных. Причина аналогичная.
- Ботам добавлено сообщение, которое показывает в чате при лимите привязок. Ранее такое сообщение было только в самом боте, отчего игроки с включённым `confirmation` могли его не заметить.
...
- При таймауте при попытке опросить HaveIBeenPwned ошибка теперь не пишется целиком, а только который лог.
### 📦 Обновлено
